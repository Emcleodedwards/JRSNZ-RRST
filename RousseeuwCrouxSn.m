...
%
% OUTPUTS: 
%   Sn = the Sn scale measurement
%   x_j = median absolute deviations from the median
%
% Example:          
%                   % basic example
%                   X = [1 5 2 2 7 4 1 6]';
...
%                   
%                   30/12/2023 EM Edit median calculation to exclude NaN values in X                            
%   
...
% 
    ...
    for i = 1:n
        ...
        x_j(i) = nanmedian(d); % compute median distance, excluding NaN values
    end

    % compute median of all median differences (excluding NaN values), and apply finite sample
    % correction, c
    Sn = c * nanmedian(x_j);
end
